...
def efficent_ADD(input,filters,stride=(1,1,1),deep=True):
    ...
    x = INC(first, filters)
    x=efficient_block(inputs=x,filters=filters*2,stride=(1,1,1))

    x = Conv3D(filters, (5, 3, 3), strides=(1, 1, 1),
               kernel_initializer='he_normal', padding='same')(x)
    x = BatchNormalization(axis=-1, momentum=0.99,epsilon=1e-3)(x)
    x=DropConnect3D(0.2)(x)  #添加进去
    x=Add()([first,x])
    return x
def Efficient(**params):
    inputs=Input((24,112,112,3))
    x = Conv3D(16, (7, 7, 7), strides=(1,2,2), padding='same',name='start')(inputs)
    ...
    l11=Bidirectional(LSTM(128,return_sequences=True,kernel_initializer='Orthogonal', name='lstm11'),
                           merge_mode='concat',name='bid11')(feature)
    l12 = Bidirectional(LSTM(128, return_sequences=True, kernel_initializer='Orthogonal', name='lstm12'),
                          merge_mode='concat', name='bid12')(l11)
    l21=Bidirectional(LSTM(128,return_sequences=True,kernel_initializer='Orthogonal', name='lstm21'),
                           merge_mode='concat',name='bid21')(l12)

    # x=concatenate([l12,l22],axis=-1)
    x = Bidirectional(GRU(256, return_sequences=True, kernel_initializer='Orthogonal', name='lstm2'),
                      merge_mode='concat', name='gru1')(l21)
    x = resconv1D(x, 128, s_list=[1,2 ,1 ,2, 1, 2,])
    # x=seq_BLOCK(x,64)
    # x = MaxPooling1D(2)(x)
    # x = seq_BLOCK(x, 128)
    # x = MaxPooling1D(2)(x)
